...
SELECT CODFAC,FECHA,DTO*2 FROM FACTURAS WHERE IVA = 0;
...
-- 4.Código de factura y de artículo de las líneas de factura en las --
--que la cantidad solicitada es menor de 5 unidades y además se ha aplicado un descuento del 25% o mayor.--
...
SELECT DESCRIP,STOCK_MIN FROM ARTICULOS WHERE (STOCK < STOCK_MIN);
...
SELECT DISTINCT CODART FROM LINEAS_FAC WHERE (CODFAC >= 8) AND (CODFAC <= 10);
...
SELECT DISTINCT CODPUE FROM CLIENTES WHERE CODCLI IS NOT NULL; 

-- 12. Obtener los códigos de los pueblos en donde hay clientes con código --
--De cliente menor que el código 25. No deben salir códigos repetidos.--
...
SELECT CODFAC,FECHA FROM FACTURAS WHERE EXTRACT(YEAR FROM FECHA) = EXTRACT(YEAR FROM SYSDATE)-1 AND (CODCLI >= 50 AND CODCLI <= 100);
...
SELECT CODFAC,FECHA FROM FACTURAS WHERE FECHA=to_date('01/06/2004','DD/MM/YYYY');
...
SELECT CODFAC,FECHA FROM FACTURAS WHERE FECHA=to_date('01/06/2004','DD/MM/YYYY') AND (CODCLI >= 100 AND CODCLI <= 250);

--19.Código y fecha de las facturas para los clientes cuyos códigos están entre 90 y 100 y no tienen iva. NOTA:--
--una factura no tiene iva cuando éste es cero o nulo.--
SELECT CODFAC,FECHA FROM FACTURAS WHERE (CODCLI >= 90 AND CODCLI <= 100) AND (IVA IS NULL OR IVA = 0);
...
SELECT NOMBRE FROM CLIENTES c WHERE CODPOSTAL LIKE '02%' OR CODPOSTAL LIKE '11%' OR CODPOSTAL LIKE '21%';
