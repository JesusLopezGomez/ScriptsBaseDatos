...
SELECT RC.COD_RESIDUO,COUNT(DISTINCT RC.COD_CONSTITUYENTE)
FROM RESIDUO_CONSTITUYENTE rc 
...
FROM RESIDUO_EMPRESA RE JOIN RESIDUO R ON RE.COD_RESIDUO=R.COD_RESIDUO 
JOIN RESIDUO_CONSTITUYENTE RC ON RC.COD_RESIDUO = R.COD_RESIDUO 
...
FROM EMPRESAPRODUCTORA E JOIN RESIDUO_EMPRESA RE
ON E.NIF_EMPRESA = RE.NIF_EMPRESA JOIN RESIDUO r 
ON RE.COD_RESIDUO = R.COD_RESIDUO
WHERE R.OD_RESIDUO LIKE '%metales%';
...
AND EXTRACT(MONTH FROM RE.FECHA) 
BETWEEN EXTRACT(MONTH FROM SYSDATE) 
AND EXTRACT(MONTH FROM SYSDATE)+1
AND LOWER(E.NOMBRE_EMPRESA) LIKE '%c%'
ORDER BY RE.FECHA,R.OD_RESIDUO DESC,RE.FECHA DESC;
